...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (my_rank == 0 || my_rank == rank_count - 1){
        ...
        if ((my_rank * (n/rank_count) - TIMEBLOCK) <= n/2 && ((my_rank + 1) * (n/rank_count) + TIMEBLOCK) >= n/2) {
            if (my_rank == 0) {
                ...
                in[n/2 - (my_rank * n/rank_count - TIMEBLOCK)] = n;
            }
        }
    } else {
        // In a middle block, we have overlap on two sides, so we allocate 2 * timeblock as buffer
        size = n/rank_count + 2*TIMEBLOCK ;
        in = calloc(n/rank_count + 2*TIMEBLOCK, sizeof(REAL));
        out = malloc((n/rank_count +2*TIMEBLOCK) * sizeof(REAL));
    ...
    double duration;
    TIME(duration, Stencil(&in, &out, size, iterations, my_rank, rank_count););
